...
if 'df_user_original' not in st.session_state:
    ...
    # Fill nulls in text columns only (not numeric columns)
    text_cols_to_fill = [
    ...
    # ── Numeric casts (only for columns that actually exist) ──────────────────
    if 'vote_average' in movies_cleaned.columns:
        movies_cleaned['vote_average'] = pd.to_numeric(
            movies_cleaned['vote_average'], errors='coerce').fillna(0).astype(int)
    else:
        movies_cleaned['vote_average'] = 0

    if 'vote_count' in movies_cleaned.columns:
        movies_cleaned['vote_count'] = pd.to_numeric(
            movies_cleaned['vote_count'], errors='coerce').fillna(0).astype(int)
    else:
        # Derive vote_count from ratings data (df_cleaned) if not in movies_cleaned
        try:
            _uid = 'userId'  if 'userId'  in ratings.columns else 'user_id'
            _mid = 'movieId' if 'movieId' in ratings.columns else 'movie_id'
            vc = (
                ratings.rename(columns={_uid: 'userId', _mid: 'movieId'})
                .groupby('movieId').size()
                .reset_index(name='vote_count')
            )
            movies_cleaned = pd.merge(movies_cleaned, vc, on='movieId', how='left')
            movies_cleaned['vote_count'] = movies_cleaned['vote_count'].fillna(0).astype(int)
        except Exception:
            movies_cleaned['vote_count'] = 999  # treat all movies as eligible
    ...
    # If ratings already has 'title' (from df_cleaned merge), use it directly
    if 'title' in ratings.columns:
        ratings_with_titles = ratings.drop(columns=['timestamp'], errors='ignore').copy()
    else:
